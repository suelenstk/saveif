...
public class Topicos {
    ...
    @RequestMapping(path = "/grupos/{idGrupo}/topicos/{idTopico}/resolvido", method = RequestMethod.PUT)
    @ResponseStatus(HttpStatus.OK)
    public void marcarComoResolvido(@AuthenticationPrincipal UsuarioAut usuarioAut, @PathVariable int idGrupo, @PathVariable int idTopico) throws Exception {

        if (grupoDAO.existsById(idGrupo) && topicoDAO.existsById(idTopico)) {

            Topico topicoResolvido = topicoDAO.findById(idTopico);
            if (topicoResolvido.getNome().equalsIgnoreCase("geral"))
                throw new Exception("O tópico geral não pode ser marcado como resolvido");

            Grupo grupo = grupoDAO.findById(idGrupo);
            boolean coordenador = false;
            List<Usuario> coordenadores;
            coordenadores = grupo.getCoordenadoresGrupo();
            ...
            if ((grupo.getDonoGrupo().getId() == usuarioAut.getUsuario().getId()) || coordenador) {
                topicoResolvido.setDataFinalizacao(new Date(System.currentTimeMillis()));
                topicoDAO.save(topicoResolvido);
            } else
                throw new Exception("Apenas o dono do grupo ou um coordenador pode marcar um tópico como resolvido.");
        } else throw new Exception("Grupo ou tópico não encontrado.");
    }
